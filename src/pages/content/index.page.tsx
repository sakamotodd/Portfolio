...
const ContentPage: NextPageWithLayout = () => {
  ...
  return (
    <div className="w-screen h-screen font-serif text-black dark:text-gray-200 bg-gray-200 dark:bg-darkCard">
      <Header title="TweetApp" darkMode={darkMode} setDarkMode={setDarkMode} />
      <div className="m-auto w-1/2 font-hiragino">
        <h1 className="py-4 text-2xl text-gray-500">投稿一覧</h1>
        <div className="grid grid-cols-2 gap-4 h-[calc(100vh-9rem-3.5rem)]">
          {data?.map((lie, index) => {
            return (
              pageDataMin <= index &&
              index < pageDataMax && (
