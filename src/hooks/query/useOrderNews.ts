...
import { GET_ORDER_NEWS, GET_PRIVATE_NEWS } from '../../GraphQL/queries';
...
export const orderNews = async () => {
  const { news: data } = await request(process.env.NEXT_PUBLIC_HASURA_ENDPOINT, GET_ORDER_NEWS);
  return data;
};
